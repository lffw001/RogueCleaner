...
- 如遇右键菜单异常、恢复中心不显示失败项或更新异常，请务必到项目问题区提交问题，并附上反馈样本（清理批次记录、截图、系统版本、复现步骤等），以便定位修复。

## v2.0.15 更新

- 修复高缩放和窄窗口下统计卡、顶部操作栏、右键管理工具栏及部分编辑窗口可能被遮挡的问题。
- 优化反馈窗口、恢复中心和旧式菜单编辑器的自适应布局，主操作保持可见、可点击。
- 新增本地白名单：保留原始证据展示，不参与批量处理。
- 新增网盘虚拟盘只读诊断：只展示盘符证据，不删除设备、盘符或客户端。
- 新增安全软件误报复核材料生成，便于用户向安全软件厂商提交复核。
- 检查更新会显示最新发布说明摘要；本版本的发布说明全部使用中文。
- 已完成本地多档缩放几何回归；不同显示设备和缩放组合仍欢迎通过项目问题区反馈。
...
每次涉及功能或界面改动前，都会只读核对 GitHub Issue 反馈，确认哪些已有证据覆盖、哪些需要修复或补充样本；公开回复或关闭 Issue 仍由作者确认。规则见 `docs/development/github-issue-feedback-triage-standard.md`。
...
程序会按天检查 GitHub Release 是否有新版本。发现新版本时会显示最新发布说明摘要，并弹窗询问是否下载并自动重启更新：
...
```text
RogueCleaner-v2.0.16-*.exe
```

压缩包主要给需要审查源码或保留完整发布包的人使用。每次打包会同时生成 `SHA256SUMS-v2.0.16-*.txt`，用于核对下载文件是否和发布文件一致。
...
这个参数会在默认窗口和最小窗口下检查“开始扫描”“清理勾选”“检查更新”“反馈”的可见性与排列，填充本地模拟结果验证右侧详情面板，并把主界面、最小窗口和反馈窗体截图写入 `reports`。它还会执行 100%/125%/150%/200% 的进程内几何模拟，检查换行、父容器裁切和按钮文字；该模拟不能代替真实 Windows 缩放/实际 `DeviceDpi` 的人工验收，在对外宣称真实高缩放已验证或关闭相关反馈前，仍须按 `docs/validation/2026-08-08-ui-dpi-release-gate.md` 完成真机矩阵。它不扫描、清理或提交反馈。
...
在结果表中右键项目，选择“加入/移除本地白名单”，即可管理本地白名单。白名单保存于 `流氓软件克星数据\state\user-whitelist.json`；命中项目仍会展示原始证据，但固定为“已白名单”、不可勾选，也不会进入批量处理。

扫描会对名称或卷标具有明确网盘特征的盘符进行只读诊断。它只展示盘符、类型、卷标和文件系统证据，绝不自动卸载网盘、删除设备、移除驱动器或改写盘符。

如需向安全软件厂商申诉误报，可运行：

```powershell
.\流氓软件克星.exe --vendor-review-smoke
```

程序会在 `流氓软件克星数据\reports` 生成包含版本、SHA-256、签名状态与生成时间的本地 Markdown 材料；不会自行提交或规避安全软件。
