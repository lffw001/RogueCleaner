...
[size=4][b]v2.0.11 更新重点[/b][/size]

1. 新增“此电脑 / 资源管理器”网盘入口图标扫描清理。
   现在会检查百度网盘、夸克网盘、WPS 云盘等常见同步入口，不只盯右键菜单。

2. 补充夸克网盘右键规则。
   包括“上传到夸克网盘”、夸克 PDF、万能转换等右键项。

3. 补充钉钉文件右键规则。
   包括右键点击文件时出现的“上传钉钉并打开”等 DingTalk / 钉盘上传入口。

4. 继续优化右键菜单、文件关联、浏览器外部宿主、服务、计划任务等扫描范围。

5. 优化高 DPI 缩放界面，Win11 200% 缩放下更好看一些。

6. 发布包改为直出 EXE，并附带 SHA256 校验，方便确认文件来源。

7. 停用自解压一键运行版，源码树也移除旧 PowerShell 引擎、旧 Launcher 和旧 rules 目录，减少安全软件启发式误报点。

8. 移除首次运行“创建桌面快捷方式”提示，不再写入桌面 `.lnk`，减少 360/火绒对快捷方式创建行为的拦截。

9. 扫描异常时会写入 `scan-error-*.json` 失败报告，反馈时不再只能靠一句“扫描失败”猜原因。

[size=4][b]下载建议[/b][/size]

普通用户优先下载 GitHub 发布页里的直出 EXE，双击就能运行：

[code]RogueCleaner-v2.0.11-20260725-150639.exe[/code]

SHA256：

[code]15de2b0847fcb0ca61098dcff2432f4c67f6d257018d90959dba1dac6190054e[/code]
...
[code]RogueCleaner-Transparent-v2.0.11-20260725-150639.rar[/code]

需要审查文件内容的，可以下载源码包：

[code]RogueCleaner-Source-v2.0.11-20260725-150639.rar[/code]

校验文件：

[code]SHA256SUMS-v2.0.11-20260725-150639.txt[/code]

普通用户不用下载源码包；从 GitHub 下载 EXE 的，放桌面或普通文件夹里双击即可。从论坛附件下载 RAR 的，先解压再运行里面的 EXE。

为避免旧附件/旧包混淆：只认 v2.0.11 这一组文件名和上面的 SHA256，旧版本文件名不建议下载。
