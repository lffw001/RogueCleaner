...
- 隐藏卸载入口：Uninstall 注册表里标记 `SystemComponent`、`NoRemove`、无显示名、无卸载命令或挂在父组件下的条目。没有卸载命令的只报告；有卸载命令的可手动勾选并弹出原厂卸载器。
- 运行进程：对弹窗、推广、守护和自动恢复组件只报告，不默认杀进程。

## 安全边界

- 主程序和“全家桶”静默卸载未默认启用。
- 隐藏/无卸载命令的组件只报告，不一键卸载。
- 有卸载命令的疑似捆绑/弹窗/守护组件只弹出原厂卸载器，不静默卸载、不替用户点确认。
- 弹出卸载器项不参与“勾选可清理”批量选择，必须用户手动勾选。
- 默认打开程序只报告，不一键改。
